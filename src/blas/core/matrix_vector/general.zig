...
const tuning = level2_tuning.active.gemv;
...
pub fn freeCurrentThreadCaches() void {
    if (gemv_workspace_f32_ptr) |ptr| std.heap.c_allocator.free(ptr[0..gemv_workspace_f32_len]);
    gemv_workspace_f32_ptr = null;
    gemv_workspace_f32_len = 0;
    if (gemv_workspace_f64_ptr) |ptr| std.heap.c_allocator.free(ptr[0..gemv_workspace_f64_len]);
    gemv_workspace_f64_ptr = null;
    gemv_workspace_f64_len = 0;
    if (gemv_workspace_c32_ptr) |ptr| std.heap.c_allocator.free(ptr[0..gemv_workspace_c32_len]);
    gemv_workspace_c32_ptr = null;
    gemv_workspace_c32_len = 0;
    if (gemv_workspace_c64_ptr) |ptr| std.heap.c_allocator.free(ptr[0..gemv_workspace_c64_len]);
    gemv_workspace_c64_ptr = null;
    gemv_workspace_c64_len = 0;
}
...
fn parallelGemvTransUnitReal(comptime T: type, m: usize, n: usize, alpha: T, a: [*]const T, lda: BlasInt, x: [*]const T, y: [*]T) bool {
    ...
    task_count = tuning.capTransTasks(task_count, n);
    const block_cols = tuning.transBlockColumns();
...
fn gemvUnitReal(comptime T: type, trans_: Order, m: usize, n: usize, alpha: T, a: [*]const T, lda: BlasInt, x: [*]const T, beta: T, y: [*]T) void {
    if (alpha == 0) {
        scaleUnitReal(T, if (trans_ == .no_trans) m else n, beta, y);
        return;
    }
    if (trans_ == .no_trans) {
        ...
        const prefer_parallel_wide_f64 = tuning.preferWideF64Parallel(T, m, n);
...
fn gemvNoTransUnitComplex(comptime T: type, m_: BlasInt, n: usize, alpha: T, a: [*]const T, lda: BlasInt, x: [*]const T, y: [*]T) void {
    ...
    for (0..n) |j| {
        ...
        if (!isZero(T, xj)) vector_ops.axpy(T, m_, xj, a + matIndex(lda, 0, j), 1, y, 1);
    }
}
...
fn gemvNoTransUnitComplexC32(m: usize, n: usize, alpha: scalar.ComplexF32, a: [*]const scalar.ComplexF32, lda: BlasInt, x: [*]const scalar.ComplexF32, y: [*]scalar.ComplexF32) void {
    var j: usize = 0;
    if (tuning.preferNoTransComplexWideColumns(m)) {
        while (j + 8 <= n) : (j += 8) {
...
const GemvNoTransColumnRange = struct {
    n0: usize,
    n1: usize,
};

fn gemvNoTransComplexTaskRange(comptime T: type, m: usize, n: usize, task_count: usize, task_index: usize, block_count: usize, block_cols: usize) GemvNoTransColumnRange {
    if (tuning.useC64BalancedColumnRanges(T, m, n, task_count)) {
        const heavy_tasks: usize = 8;
        const heavy_cols: usize = 52;
        const light_cols: usize = 48;
        if (task_index < heavy_tasks) {
            const n0 = task_index * heavy_cols;
            return .{ .n0 = n0, .n1 = n0 + heavy_cols };
        }
        const n0 = heavy_tasks * heavy_cols + (task_index - heavy_tasks) * light_cols;
        return .{ .n0 = n0, .n1 = n0 + light_cols };
    }
    if (block_count > 0) {
        return .{
            .n0 = (task_index * block_count / task_count) * block_cols,
            .n1 = if (task_index + 1 == task_count) n else ((task_index + 1) * block_count / task_count) * block_cols,
        };
    }
    return .{
...
fn GemvNoTransRowsComplexTask(comptime T: type) type {
    return struct {
        row_count: usize,
        n: usize,
        alpha: T,
...
fn runGemvNoTransRowsComplexTask(comptime T: type, raw_tasks: *const anyopaque, index: usize) void {
    const tasks: [*]const GemvNoTransRowsComplexTask(T) = @ptrCast(@alignCast(raw_tasks));
    const task = tasks[index];
    _ = matrix_vector_kernels.gemvNoTransRowsUnitComplex(T, task.row_count, task.n, task.alpha, task.a, task.lda, task.x, task.y);
}

fn runGemvNoTransRowsComplexTaskC64(raw_tasks: *const anyopaque, index: usize) void {
    runGemvNoTransRowsComplexTask(scalar.ComplexF64, raw_tasks, index);
}

fn GemvNoTransTiledFullComplexTask(comptime T: type) type {
    return struct {
        n: usize,
        alpha: T,
        a: [*]const T,
...
fn runGemvNoTransTiledFullComplexTask(comptime T: type, raw_tasks: *const anyopaque, index: usize) void {
    const tasks: [*]const GemvNoTransTiledFullComplexTask(T) = @ptrCast(@alignCast(raw_tasks));
    const task = tasks[index];
    var col: usize = 0;
    var beta_tile = task.beta;
    while (col < task.n) : (col += 128) {
        const ran = matrix_vector_kernels.gemvNoTransFullUnitComplex(T, 128, 128, task.alpha, task.a + matIndex(task.lda, 0, col), task.lda, task.x + col, beta_tile, task.y);
        std.debug.assert(ran);
        beta_tile = one(T);
    }
}

fn runGemvNoTransTiledFullComplexTaskC64(raw_tasks: *const anyopaque, index: usize) void {
    runGemvNoTransTiledFullComplexTask(scalar.ComplexF64, raw_tasks, index);
}

fn runGemvNoTransTiledFullComplexTaskC32(raw_tasks: *const anyopaque, index: usize) void {
    runGemvNoTransTiledFullComplexTask(scalar.ComplexF32, raw_tasks, index);
}

fn parallelGemvNoTransTiledFullComplex(comptime T: type, m: usize, n: usize, alpha: T, a: [*]const T, lda: BlasInt, x: [*]const T, beta: T, y: [*]T) bool {
    if (!tuning.preferComplexTiledFull(T, m, n)) return false;
    if (!matrix_vector_kernels.supportsGemvNoTransFullUnitComplex(T, 128, 128, lda)) return false;

    const row_tiles = m / 128;
    var tasks: [4]GemvNoTransTiledFullComplexTask(T) = undefined;
    for (0..row_tiles) |task_index| {
        const row0 = task_index * 128;
        tasks[task_index] = .{
            .n = n,
            .alpha = alpha,
            .a = a + row0,
    ...
    const runner = if (T == scalar.ComplexF32) runGemvNoTransTiledFullComplexTaskC32 else runGemvNoTransTiledFullComplexTaskC64;
    return core_pool.runLowLatency(runner, @ptrCast(&tasks), row_tiles);
}

fn parallelGemvNoTransRowsUnitComplex(comptime T: type, m: usize, n: usize, alpha: T, a: [*]const T, lda: BlasInt, x: [*]const T, y: [*]T) bool {
    if (!tuning.preferNoTransRowsComplex(T, m, n, lda)) return false;

    const row_count: usize = 64;
    if (!matrix_vector_kernels.supportsGemvNoTransRowsUnitComplex(T, row_count, n, lda)) return false;

    var tasks: [8]GemvNoTransRowsComplexTask(T) = undefined;
    for (0..tasks.len) |task_index| {
        const row0 = task_index * row_count;
        tasks[task_index] = .{
            .row_count = row_count,
            .n = n,
            .alpha = alpha,
    ...
    return core_pool.runLowLatency(runGemvNoTransRowsComplexTaskC64, @ptrCast(&tasks), tasks.len);
}
...
fn parallelGemvNoTransUnitComplex(comptime T: type, m: usize, n: usize, m_: BlasInt, alpha: T, a: [*]const T, lda: BlasInt, x: [*]const T, y: [*]T) bool {
    ...
    if (!tuning.workspaceAllowed(workspace_len, @sizeOf(T))) return false;
    const workspace = gemvWorkspace(T, workspace_len) orelse return false;
    const block_cols = tuning.complexBlockColumns();
    const block_count = n / block_cols;
    var task_overwrites_workspace = (T == scalar.ComplexF32 or T == scalar.ComplexF64) and m == 512 and n == 512;
    if (task_overwrites_workspace) {
        for (0..task_count) |task_index| {
            const range = gemvNoTransComplexTaskRange(T, m, n, task_count, task_index, block_count, block_cols);
            if (!matrix_vector_kernels.supportsGemvNoTransTaskUnitComplex(T, 512, range.n1 - range.n0, lda)) {
                task_overwrites_workspace = false;
                break;
            }
        }
    }
    if (!task_overwrites_workspace) @memset(workspace, zero(T));
    ...
    if (task_overwrites_workspace) {
        if (T == scalar.ComplexF32) {
            const real_y: [*]f32 = @ptrCast(y);
            for (0..task_count) |task_index| {
                const real_delta: [*]const f32 = @ptrCast(workspace.ptr + task_index * m);
                addUnitReal(f32, 2 * m, real_delta, real_y);
            }
            return true;
        }
        if (T == scalar.ComplexF64) {
            const real_y: [*]f64 = @ptrCast(y);
            for (0..task_count) |task_index| {
                const real_delta: [*]const f64 = @ptrCast(workspace.ptr + task_index * m);
                addUnitReal(f64, 2 * m, real_delta, real_y);
            }
            return true;
        }
    }
    ...
    for (0..task_count) |task_index| {
        vector_ops.axpy(T, m_, add_alpha, workspace.ptr + task_index * m, 1, y, 1);
    }
    return true;
}
...
fn gemvTransUnitComplex(comptime T: type, m_: BlasInt, n: usize, alpha: T, a: [*]const T, lda: BlasInt, x: [*]const T, y: [*]T, do_conj: bool) void {
    ...
    for (0..n) |j| {
        const sum = vector_ops.dot(T, m_, a + matIndex(lda, 0, j), 1, x, 1, do_conj);
...
fn gemvTransUnitComplexC32(m: usize, n: usize, alpha: scalar.ComplexF32, a: [*]const scalar.ComplexF32, lda: BlasInt, x: [*]const scalar.ComplexF32, y: [*]scalar.ComplexF32, do_conj: bool) void {
    var j: usize = 0;
    if (tuning.preferTransComplexWideColumns(scalar.ComplexF32, m)) {
        while (j + 4 <= n) : (j += 4) {
...
fn gemvTransUnitComplexC64(m: usize, n: usize, alpha: scalar.ComplexF64, a: [*]const scalar.ComplexF64, lda: BlasInt, x: [*]const scalar.ComplexF64, y: [*]scalar.ComplexF64, do_conj: bool) void {
    var j: usize = 0;
    if (tuning.preferTransComplexWideColumns(scalar.ComplexF64, m)) {
        while (j + 4 <= n) : (j += 4) {
...
fn GemvTransFullComplexTask(comptime T: type) type {
    return struct {
        alpha: T,
        a: [*]const T,
        lda: BlasInt,
...
fn runGemvTransComplexTask(comptime T: type, raw_tasks: *const anyopaque, index: usize) void {
    ...
    const task_m = toUsize(task.m);
    const task_n = task.n1 - task.n0;
    if (tuning.preferC64TransTask(T, task_m, task_n, task.do_conj)) {
...
fn runGemvTransTiledFullComplexTask(comptime T: type, raw_tasks: *const anyopaque, index: usize) void {
    ...
    if (comptime T == scalar.ComplexF64) {
        if (tuning.preferC64TransTiledTask(T, task.m, task.do_conj)) {
...
fn runGemvTransTiledFullComplexTaskC64(raw_tasks: *const anyopaque, index: usize) void {
    runGemvTransTiledFullComplexTask(scalar.ComplexF64, raw_tasks, index);
}

fn runGemvTransTiledFullComplexTaskC32(raw_tasks: *const anyopaque, index: usize) void {
    runGemvTransTiledFullComplexTask(scalar.ComplexF32, raw_tasks, index);
}
...
fn parallelGemvTransTiledFullComplex(comptime T: type, m: usize, n: usize, alpha: T, a: [*]const T, lda: BlasInt, x: [*]const T, beta: T, y: [*]T, do_conj: bool) bool {
    ...
    const col_tiles = n / 128;
    var tasks: [4]GemvTransTiledFullComplexTask(T) = undefined;
    for (0..col_tiles) |task_index| {
        const col0 = task_index * 128;
        tasks[task_index] = .{
            .m = m,
            .alpha = alpha,
            .a = a + matIndex(lda, 0, col0),
            .lda = lda,
            .x = x,
            .beta = beta,
            .y = y + col0,
    ...
    const runner = if (T == scalar.ComplexF32) runGemvTransTiledFullComplexTaskC32 else runGemvTransTiledFullComplexTaskC64;
    return core_pool.runLowLatency(runner, @ptrCast(&tasks), col_tiles);
}
...
fn parallelGemvTransTaskFullComplex(comptime T: type, m: usize, n: usize, alpha: T, a: [*]const T, lda: BlasInt, x: [*]const T, beta: T, y: [*]T, do_conj: bool) bool {
    if (!tuning.preferTransTaskFullComplex(T, m, n)) return false;
    const task_n: usize = 64;
    if (!matrix_vector_kernels.supportsGemvTransTaskFullUnitComplex(T, m, task_n, lda, do_conj)) return false;

    var tasks: [8]GemvTransFullComplexTask(T) = undefined;
    for (0..tasks.len) |task_index| {
        const n0 = task_index * task_n;
        tasks[task_index] = .{
            .alpha = alpha,
            .a = a + matIndex(lda, 0, n0),
            .lda = lda,
            .x = x,
            .beta = beta,
            .y = y + n0,
    ...
    return core_pool.runLowLatency(runGemvTransFullComplexTaskC64, @ptrCast(&tasks), tasks.len);
}
...
fn gemvUnitComplex(comptime T: type, trans_: Order, m_: BlasInt, n_: BlasInt, alpha: T, a: [*]const T, lda: BlasInt, x: [*]const T, beta: T, y: [*]T) void {
    ...
    const beta_zero = isZero(T, beta);
    if (beta_zero) @memset(y[0..toUsize(leny)], zero(T));
    if (trans_ == .no_trans and !isZero(T, alpha)) {
        if (matrix_vector_kernels.gemvNoTransFullUnitComplex(T, m, n, alpha, a, lda, x, beta, y)) return;
        if (parallelGemvNoTransTiledFullComplex(T, m, n, alpha, a, lda, x, beta, y)) return;
    } else if (!isZero(T, alpha)) {
        const do_conj = trans_ == .conj_trans;
        if (matrix_vector_kernels.gemvTransFullUnitComplex(T, m, n, alpha, a, lda, x, beta, y, do_conj)) return;
        if (parallelGemvTransTiledFullComplex(T, m, n, alpha, a, lda, x, beta, y, do_conj)) return;
        if (parallelGemvTransTaskFullComplex(T, m, n, alpha, a, lda, x, beta, y, do_conj)) return;
    }
    if (!beta_zero) vector_ops.scal(T, leny, beta, y, 1);
...
pub fn gemv(comptime T: type, trans_: Order, m_: BlasInt, n_: BlasInt, alpha: T, a: [*]const T, lda: BlasInt, x: [*]const T, incx_: BlasInt, beta: T, y: [*]T, incy_: BlasInt) void {
    ...
    const m = toUsize(m_);
    const n = toUsize(n_);
    const selected = level2_tuning.selectDefault(T, .gemv, .{
        .m = m,
        .n = n,
        .incx = incx_,
        .incy = incy_,
    });
    if (selected.kernel.implementation == .core_unit) {
        if (comptime isReal(T)) return gemvUnitReal(T, trans_, m, n, alpha, a, lda, x, beta, y);
        if (comptime isComplex(T)) return gemvUnitComplex(T, trans_, m_, n_, alpha, a, lda, x, beta, y);
    }
...
pub fn gbmv(comptime T: type, trans_: Order, m_: BlasInt, n_: BlasInt, kl_: BlasInt, ku_: BlasInt, alpha: T, a: [*]const T, lda: BlasInt, x: [*]const T, incx_: BlasInt, beta: T, y: [*]T, incy_: BlasInt) void {
    ...
    if (incx_ == 1 and incy_ == 1 and
        tuning.preferGbmvUnit(T, trans_ == .conj_trans, m, n, band_width))
    {
        const leny = if (trans_ == .no_trans) m else n;
...
pub const testing = struct {
    pub fn gemvPanel(
        ...
        if (comptime isReal(T)) {
            if (trans_ == .no_trans) return gemvNoTransUnitReal(T, m, n, alpha, a, lda, x, y);
            return gemvTransUnitReal(T, m, n, alpha, a, lda, x, y);
        }
        if (trans_ == .no_trans) return gemvNoTransUnitComplex(T, @intCast(m), n, alpha, a, lda, x, y);
        return gemvTransUnitComplex(T, @intCast(m), n, alpha, a, lda, x, y, trans_ == .conj_trans);
    }
};
