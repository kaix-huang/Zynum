...
fn parallelSymvUnitReal(comptime T: type, uplo: Uplo, n: usize, alpha: T, a: [*]const T, lda: BlasInt, x: [*]const T, y: [*]T) bool {
    ...
    const min_cols_per_task: usize = if (n >= 768) 64 else 128;
...
fn hermvColumnsUnitComplex(comptime T: type, uplo: Uplo, n: usize, j0: usize, j1: usize, alpha: T, a: [*]const T, lda: BlasInt, x: [*]const T, y_delta: [*]T) void {
    if (uplo == .upper) {
        for (j0..j1) |j| {
            ...
            if (j > 0) {
                if (!isZero(T, xj)) level1.axpy(T, @intCast(j), xj, col, 1, y_delta, 1);
                const sum = level1.dot(T, @intCast(j), col, 1, x, 1, true);
        ...
        for (j0..j1) |j| {
            ...
            if (j + 1 < n) {
                ...
                const tail_x = x + j + 1;
                if (!isZero(T, xj)) level1.axpy(T, @intCast(tail_len), xj, tail_col, 1, y_delta + j + 1, 1);
                const sum = level1.dot(T, @intCast(tail_len), tail_col, 1, tail_x, 1, true);
                y_delta[j] = add(T, y_delta[j], mul(T, alpha, sum));
            }
        }
    }
...
fn parallelHermvUnitComplex(comptime T: type, uplo: Uplo, n: usize, n_: BlasInt, alpha: T, a: [*]const T, lda: BlasInt, x: [*]const T, y: [*]T) bool {
    ...
    const balance_upper_c64 = T == scalar.ComplexF64 and n == 512 and uplo == .upper;
    if (balance_upper_c64) task_count = @min(task_count, 8);
    if (task_count <= 1) return false;
    ...
    for (0..task_count) |task_index| {
        const j0 = if (balance_upper_c64) upperTriangularTaskBoundary(n, task_count, task_index) else task_index * n / task_count;
        const j1 = if (balance_upper_c64) upperTriangularTaskBoundary(n, task_count, task_index + 1) else (task_index + 1) * n / task_count;
        tasks[task_index] = .{
            .uplo = uplo,
            .n = n,
            .j0 = j0,
    ...
    for (0..task_count) |task_index| {
        level1.axpy(T, n_, add_alpha, workspace.ptr + task_index * n, 1, y, 1);
    }
    return true;
}
