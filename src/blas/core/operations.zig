...
pub fn swapVectors(comptime T: type, args: operands.VectorSwap(T)) void {
    ...
    level1.swap(T, length, args.first.values, args.first.stride, args.second.values, args.second.stride);
}
...
pub fn copyVector(comptime T: type, args: operands.VectorCopy(T)) void {
    ...
    level1.copy(T, length, args.source.values, args.source.stride, args.destination.values, args.destination.stride);
}

pub fn scaleVector(comptime T: type, args: operands.VectorScale(T)) void {
    level1.scal(T, args.vector.length, args.scale, args.vector.values, args.vector.stride);
}

pub fn addScaledVector(comptime T: type, args: operands.ScaledVectorAdd(T)) void {
    const length = sharedLength(args.source.length, args.destination.length);
    level1.axpy(T, length, args.scale, args.source.values, args.source.stride, args.destination.values, args.destination.stride);
}

pub fn combineVectors(comptime T: type, args: operands.VectorLinearCombination(T)) void {
    const length = sharedLength(args.source.length, args.destination.length);
    level1.axpby(T, length, args.source_scale, args.source.values, args.source.stride, args.destination_scale, args.destination.values, args.destination.stride);
}
...
pub fn dotProduct(comptime T: type, args: operands.VectorDot(T)) T {
    ...
    return level1.dot(T, length, args.left.values, args.left.stride, args.right.values, args.right.stride, args.conjugate_left);
}

pub fn euclideanNorm(comptime T: type, vector: operands.ConstVector(T)) scalar.Real(T) {
    return level1.nrm2(T, vector.length, vector.values, vector.stride);
}
