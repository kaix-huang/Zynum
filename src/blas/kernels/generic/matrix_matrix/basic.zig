...
const epilogue = @import("../../matrix_matrix/epilogue.zig");
...
fn kernelVec(comptime T: type, comptime cfg: Config, task: gemm_task.Task(T), row: usize, col: usize) void {
...
fn kernelScalarRows(comptime T: type, comptime cfg: Config, task: gemm_task.Task(T), row_start: usize, col: usize) void {
    var row = row_start;
    while (row < task.m) : (row += 1) {
        ...
        inline for (0..cfg.tile_n) |n_lane| {
            task.c[matIndex(task.ldc, row, col + n_lane)] = applyScalar(T, task, acc[n_lane], row, col + n_lane);
        }
    }
}

fn kernelScalarTailColumns(comptime T: type, task: gemm_task.Task(T), col: usize) void {
    var j = col;
    while (j < task.n1) : (j += 1) {
        var row: usize = 0;
        while (row < task.m) : (row += 1) {
            ...
            task.c[matIndex(task.ldc, row, j)] = applyScalar(T, task, acc, row, j);
        }
    }
}
...
pub fn rowVectorColumns(comptime T: type, task: gemm_task.Task(T)) void {
    ...
    while (j < task.n1) : (j += 1) {
        ...
        task.c[matIndex(task.ldc, 0, j)] = applyScalar(T, task, acc, 0, j);
    }
}
...
pub fn noTransReal(comptime T: type, comptime cfg: Config, task: gemm_task.Task(T)) void {
    ...
    while (col + cfg.tile_n <= task.n1) : (col += cfg.tile_n) {
        ...
        while (row + cfg.vector_lanes <= task.m) : (row += cfg.vector_lanes) {
            kernelVec(T, cfg, task, row, col);
        }
        kernelScalarRows(T, cfg, task, row, col);
    }
    kernelScalarTailColumns(T, task, col);
}
