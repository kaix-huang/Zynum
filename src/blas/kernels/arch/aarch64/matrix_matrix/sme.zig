...
const packing = @import("../../../shared/matrix_matrix/packing.zig");
const asimd = @import("asimd.zig");
const amx = @import("amx.zig");
const features = @import("../features.zig");
const sme_asm = @import("sme_asm.zig");
...
pub fn freeCurrentThreadCaches() void {
    if (sme_f32_pack_ptr) |ptr| std.heap.c_allocator.free(ptr[0..sme_f32_pack_len]);
    sme_f32_pack_ptr = null;
    sme_f32_pack_len = 0;
    if (sme_f64_pack_ptr) |ptr| std.heap.c_allocator.free(ptr[0..sme_f64_pack_len]);
    sme_f64_pack_ptr = null;
    sme_f64_pack_len = 0;
    amx.freeCurrentThreadCaches();
}
...
pub fn tryNoTransRealF32Fast(m_: gemm_task.BlasInt, n_: gemm_task.BlasInt, k_: gemm_task.BlasInt, alpha: f32, a: [*]const f32, lda_: gemm_task.BlasInt, b: [*]const f32, ldb_: gemm_task.BlasInt, beta: f32, c: [*]f32, ldc_: gemm_task.BlasInt) bool {
    ...
    const pack_plan = gemm_task.PackWorkspacePlan{
        .stack_bytes = 128 * 1024,
        .cache_bytes = 8 * 1024 * 1024,
    };
    return amx.sgemmN32(m_, n_, k_, a, lda_, b, ldb_, c, ldc_, pack_plan) != 0;
}
...
fn plannedStackPackElems(comptime T: type, task: gemm_task.Task(T)) usize {
    const planned_bytes = if (task.execution.pack.stack_bytes == 0) maxSmeStackPackElems(T) * @sizeOf(T) else task.execution.pack.stack_bytes;
    return @min(@as(usize, maxSmeStackPackElems(T)), planned_bytes / @sizeOf(T));
}
...
fn directPackElems(comptime T: type, task: gemm_task.Task(T), tile: usize) usize {
    ...
    const panel2_cols = tile * 2;
    if (n < panel2_cols) return task.k * tile;
    const panel2_count = n / panel2_cols;
...
fn packBPanelF32(task: gemm_task.Task(f32), j: usize, tile: usize, b_pack: []f32) void {
    packing.packBPanelWithDefault(f32, 16, task, j, tile, b_pack);
}
...
fn packBPanel2F32(task: gemm_task.Task(f32), j: usize, tile: usize, b_pack: []f32) void {
    packing.packBPanelWithDefault(f32, 32, task, j, tile * 2, b_pack);
}
...
fn noTransRealF32SmeDirectWithPack(task: gemm_task.Task(f32), tile: usize, b_pack: []f32) void {
    ...
    while (j + panel2_cols <= task.n1) {
        ...
        if (full_rows_2m != 0) {
            ...
            if (task.execution.f32_panel == .panels2x2_u4 and canUseF32Panels2x2U4(task, tile)) {
                callSmeGemmPanelBatch(f32, sme_asm.sgemmPanels2x2U4F32, task.a, b_pack.ptr, c_panel, full_rows_2m, task.k, lda_bytes, ldc_bytes, batch_panels);
            } else if (batch_panels == 1) {
                callSmeGemmPanel(f32, sme_asm.sgemmPanel2x2F32, task.a, b_pack.ptr, c_panel, full_rows_2m, task.k, lda_bytes, ldc_bytes);
            } else {
                callSmeGemmPanelBatch(f32, sme_asm.sgemmPanels2x2F32, task.a, b_pack.ptr, c_panel, full_rows_2m, task.k, lda_bytes, ldc_bytes, batch_panels);
            }
        }
        ...
        if (full_rows_2m < full_rows) {
            ...
            if (batch_panels == 1) {
                callSmeGemmPanel(f32, sme_asm.sgemmPanel1x2F32, a_panel, b_pack.ptr, c_panel, full_rows - full_rows_2m, task.k, lda_bytes, ldc_bytes);
            } else {
                ...
                while (residual_panel_index < batch_panels) : (residual_panel_index += 1) {
                    ...
                    callSmeGemmPanel(f32, sme_asm.sgemmPanel1x2F32, a_panel, b_pack.ptr + panel_offset, panel_c, full_rows - full_rows_2m, task.k, lda_bytes, ldc_bytes);
                    tail_j += panel2_cols;
                }
            }
    ...
    while (j + tile <= task.n1) : (j += tile) {
        ...
        if (full_rows_4m != 0) {
            ...
            callSmeGemmPanel(f32, sme_asm.sgemmPanel4mF32, task.a, b_pack.ptr, c_panel, full_rows_4m, task.k, lda_bytes, ldc_bytes);
        }
...
fn noTransRealF64SmeDirectWithPack(task: gemm_task.Task(f64), tile: usize, b_pack: []f64) void {
    ...
    while (j + panel2_cols <= task.n1) {
        ...
        if (full_rows_4m != 0) {
            const c_panel = task.c + matIndex(task.ldc, 0, batch_j);
            if (batch_panels == 1) {
                callSmeGemmPanel(f64, sme_asm.dgemmPanel4x2F64, task.a, b_pack.ptr, c_panel, full_rows_4m, task.k, lda_bytes, ldc_bytes);
            } else {
                callSmeGemmPanelBatch(f64, sme_asm.dgemmPanels4x2F64, task.a, b_pack.ptr, c_panel, full_rows_4m, task.k, lda_bytes, ldc_bytes, batch_panels);
            }
        }
        ...
        while (panel_index < batch_panels) : (panel_index += 1) {
            ...
            if (full_rows_4m < full_rows_2m) {
                ...
                callSmeGemmPanel(f64, sme_asm.dgemmPanel2x2F64, a_panel, b_pack.ptr + panel_offset, c_panel, full_rows_2m - full_rows_4m, task.k, lda_bytes, ldc_bytes);
            }
    ...
    while (j + tile <= task.n1) : (j += tile) {
        ...
        if (full_rows_4m != 0) {
            const c_panel = task.c + matIndex(task.ldc, 0, j);
            callSmeGemmPanel(f64, sme_asm.dgemmPanel4mF64, task.a, b_pack.ptr, c_panel, full_rows_4m, task.k, lda_bytes, ldc_bytes);
        }
        if (full_rows_4m < full_rows) {
            const a_panel = task.a + matIndex(task.lda, full_rows_4m, 0);
            const c_panel = task.c + matIndex(task.ldc, full_rows_4m, j);
            callSmeGemmPanel(f64, sme_asm.dgemmPanelF64, a_panel, b_pack.ptr, c_panel, full_rows - full_rows_4m, task.k, lda_bytes, ldc_bytes);
        }
