...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description=(
            "Compare checked SYMM/HEMM report rows against the fastest requested "
            "BLAS using fresh-process statistics."
        )
    )
    parser.add_argument("csv", help="Path to a run_symm_report CSV file.")
...
def describe_key(key):
    ...
    return (
        f"{routine} {kind} shape={shape} m={m} n={n} side={side} uplo={uplo} "
        f"alpha={alpha_re},{alpha_im} beta={beta_re},{beta_im} order={order} "
        f"lda={lda} ldb={ldb} ldc={ldc} reps={reps} "
        f"process_repeats={process_repeats} metric={metric}"
    )
    ...
    with csv_file:
        for row in csv.DictReader(csv_file):
            try:
                ...
                print(
                    f"SYMM CSV row is missing required parameters: {row}",
                    file=sys.stderr,
                )
                return 2
            if not allowed:
            ...
            if not args.allow_unchecked and not checked:
                if row.get("library") == args.zynum:
                    print(
                        f"unchecked SYMM row is not eligible for comparison: {row}",
                        file=sys.stderr,
                    )
                    return 2
                continue
            try:
                ...
                print(
                    f"bad SYMM metric or parameter value in row: {row}",
                    file=sys.stderr,
                )
                return 2
    ...
    for key, by_library in sorted(groups.items()):
        ...
        if args.stat == "paired-median":
            try:
                ...
                print(
                    f"bad paired SYMM samples for {describe_key(key)}: {exc}",
                    file=sys.stderr,
                )
                return 2
        else:
            ...
            try:
                ...
                print(
                    f"bad SYMM comparison ratio for {describe_key(key)}: {exc}",
                    file=sys.stderr,
                )
                return 2
        if ratio < args.ratio:
