...
def draw_legend(svg, x, y, libraries):
    for i, lib in enumerate(libraries):
        ...
        svg.append(
            f'<rect x="{lx}" y="{y - 12}" width="18" height="12" fill="{color}"/>'
        )
        svg.append(
            f'<text x="{lx + 26}" y="{y - 2}" class="legend">{html.escape(lib)}</text>'
        )
...
def draw_group_panel(svg, rows, group, top, left, width, height):
    ...
    ticks = positive_finite_axis_ticks(max_value, padding=1.08)
    max_tick = ticks[-1]

    svg.append(
        f'<text x="{left}" y="{top - 20}" class="panel-title">{html.escape(GROUP_TITLES.get(group, group))}</text>'
    )
    for tick in ticks:
        ...
        svg.append(
            f'<text x="{left - 10}" y="{y + 4:.1f}" text-anchor="end" class="axis-label">{tick:g}</text>'
        )
    svg.append(
    ...
    for op_index, op in enumerate(ops):
        ...
        for lib_index, lib in enumerate(libs):
            ...
            color = COLORS.get(lib, "#6b7280")
            svg.append(
                f'<rect x="{x:.1f}" y="{y:.1f}" width="{bar_width - 2:.1f}" height="{h:.1f}" fill="{color}"/>'
            )
...
def render_bars(rows):
    ...
    top0 = 120
    height = (
        top0 + len(groups) * panel_height + max(0, len(groups) - 1) * panel_gap + 80
    )
    left = 105
    ...
    copy_sizes = sorted(
        {
            row["copy_bytes"]
            for row in rows
            if row["group"] == "copy" and row["copy_bytes"]
        }
    )
    seconds_values = sorted({row["seconds"] for row in rows})
    svg.append(
        '<text x="40" y="42" class="title">Zynum Level 1 Performance Coverage</text>'
    )
    ...
    for index, group in enumerate(groups):
        ...
        draw_group_panel(
            svg, by_group[group], group, top, left, chart_width, panel_height
        )
    svg.append("</svg>\n")
        ...
        result.append(
            {
                ...
                "ratio": positive_finite_ratio(zynum, best),
                "zynum": zynum,
...
def render_ratio(rows, ratios=None):
    ...
    chart_height = 270
    max_ratio = max([1.25] + [row["ratio"] for row in ratios])
    ticks = positive_finite_axis_ticks(max_ratio, padding=1.05)
    max_tick = ticks[-1]
    svg = svg_header(width, height)
    svg.append(
    ...
    svg.append(
        f'<line x1="{left}" y1="{one_y:.1f}" x2="{left + chart_width}" y2="{one_y:.1f}" stroke="#111827" stroke-width="1.5" stroke-dasharray="5 4"/>'
    )
    svg.append(
    ...
    if ratios:
        ...
        for index, row in enumerate(ratios):
            ...
            svg.append(
                f'<rect x="{cx - bar_width / 2:.1f}" y="{y:.1f}" width="{bar_width:.1f}" height="{h:.1f}" fill="{color}"/>'
            )
            svg.append(
            ...
            svg.append(
                f'<text x="{cx:.1f}" y="{y - 4:.1f}" text-anchor="middle" class="value-label">{ratio:.2f}</text>'
            )
    svg.append("</svg>\n")
    return "\n".join(svg).encode("utf-8")
